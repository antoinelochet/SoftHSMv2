...
{
	return SLHDSAPublicKey::isOfType(inType) || (inType && !strcmp(OSSLSLHDSAPublicKey::type, inType));
}
...
void OSSLSLHDSAPublicKey::setParameterSet(unsigned long inParameterSet)
{
	SLHDSAPublicKey::setParameterSet(inParameterSet);
	if (pkey != NULL)
	{
		EVP_PKEY_free(pkey);
		pkey = NULL;
