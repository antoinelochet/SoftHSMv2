...
{
	DEBUG_MSG("OSSLMLKEM::generateKeyPair() called");
	// Check parameters
	if ((ppKeyPair == NULL) ||
		(inParameters == NULL))
	{
	...
	ossl_params[0] = OSSL_PARAM_construct_int(OSSL_PKEY_PARAM_ML_KEM_RETAIN_SEED, &retainSeed);
	ossl_params[1] = OSSL_PARAM_construct_end();
	EVP_PKEY_CTX_set_params(ctx, ossl_params);
	...
	*ppKeyPair = kp;
	...
	// Release the key
	EVP_PKEY_free(pkey);
	DEBUG_MSG("OSSLMLKEM::generateKeyPair() successfully generated a key pair with parameter set: %lu", parameterSet);
	return true;
}
