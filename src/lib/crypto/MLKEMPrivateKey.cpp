...
{
	ByteString dSeed = ByteString::chainDeserialise(serialised);
	ByteString dValue = ByteString::chainDeserialise(serialised);

	const size_t valueLen = dValue.size();
	if (dSeed.size() != 64 ||
	...
	DEBUG_MSG("-- deserialise() MLKEM private key value length: %lu", dValue.size());
