...
{
	DEBUG_MSG("OSSLMLKEMPrivateKey::PKCS8Encode() called");
	ByteString der;
	...
	DEBUG_MSG("OSSLMLKEMPrivateKey::PKCS8Encode() successfully encoded private key into PKCS#8 DER format, length: %zu, value: %s", der.size(), der.hex_str().c_str());
	return der;
}
...
{
	DEBUG_MSG("OSSLMLKEMPrivateKey::PKCS8Decode() called with BER data of length: %zu, value: %s", ber.size(), ber.hex_str().c_str());
	int len = ber.size();
	...
	DEBUG_MSG("OSSLMLKEMPrivateKey::PKCS8Decode() setFromOSSL returned: %s", ok ? "true" : "false");
	return ok;
}
