...
{
	return SLHDSAPrivateKey::isOfType(inType) || (inType && !strcmp(type, inType));
}
 ...
 * Overrides setParameterSet() to clear cached pkey, mirroring setValue()'s logic,
 * so getOSSLKey() and createOSSLKey() will recreate the OpenSSL key.
 */
void OSSLSLHDSAPrivateKey::setParameterSet(unsigned long inParameterSet)
{
	SLHDSAPrivateKey::setParameterSet(inParameterSet);
	if (pkey != NULL)
	{
		EVP_PKEY_free(pkey);
		pkey = NULL;
...
void OSSLSLHDSAPrivateKey::createOSSLKey()
{
	...
	*p = OSSL_PARAM_construct_end();
	...
	EVP_PKEY *tmp = NULL;
	rv = EVP_PKEY_fromdata(ctx, &tmp, selection, params);
	if (rv <= 0)
	{
